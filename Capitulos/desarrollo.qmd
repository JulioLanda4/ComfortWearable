# Desarrollo, construcción y validación


En esta sección se describe el proceso completo de desarrollo, construcción y validación del dispositivo presentado en esta tesis. La @sec-metodología aborda la metodología general del proyecto, incluyendo una descripción detallada del dispositivo, la selección de los componentes y la justificación de su elección. En la @sec-construccion se desarrolla la etapa de diseño y construcción del dispositivo. La @sec-encuestas se centra en el diseño y la implementación de las encuestas mediante la interfaz gráfica, mientras que la @sec-calibración detalla el proceso de calibración de los sensores, para asegurar fiabilidad en las mediciones. En la @sec-programación se presenta la lógica de programación del dispositivo, y en la @sec-instrucciones se proporcionan instrucciones de uso. Finalmente, en la @sec-validación comprueba la funcionalidad del dispositivo para recopilar y almacenar información de confort térmico.
...
| Mínimo               | 32.36       | 32.80       |
| Máximo               | 35.83       | 35.83       |
: Estadísticas de las Mediciones del Sensor GY-906 y la Cámara Termográfica Fluke Ti9  {#tbl-estadisticas camara}

```{python}
#| echo: false
#| fig-cap: "Gráfico de Dispersión de las Mediciones del Sensor GY-906 y la Cámara Termográfica Fluke Ti9"
...
ax.plot(px,py,"r--", label="Línea de identidad")

ax.scatter(datos["Sensor"], datos["Camara"], s=25, alpha=0.4,label="Datos")

# Configurar la leyenda y los ejes
ax.set_xlabel("Sensor GY-906 [°C]")
ax.set_ylabel("Cámara Fluke Ti9 [°C]")
ax.grid()

# Calcular los límites de la gráfica para el primer subplot
...
El análisis de los resultados revela un error sistemático, ya que el sensor GY-906 tiende a medir temperaturas ligeramente inferiores a las registradas por la cámara termográfica. Las estadísticas de los errores son las siguientes: 

- Error Promedio (EM): -0.72°C
- Error Absoluto Medio (EAM): 0.77°C

La @fig-histograma muestra un histograma del error de medición, donde se puede observa el error sistemático del sensor respecto a la cámara. Se observa que la mayoría de los errores se concentran entre 0 y 0.5°C.

```{python}
#| echo: false
#| fig-cap: "Histograma de errores entre las mediciones de la cámara Fluke Ti9 y el sensor GY-906"
...
Para garantizar que la calibración del sensor GY-906 no utilice datos que puedan distorsionar el análisis, se utiliza el método de la envolvente elíptica, con el que se asegura que solo los datos más representativos y precisos sean utilizados para el proceso de calibración del sensor. Con este método se detectan siete valores atípicos o outliers los cuales son excluidos del conjunto de datos para el proceso de calibración. En la @fig-outliers se observan los valores atípicos detectados y excluidos.

```{python}
#| echo: false
#| fig-cap: "Valores atípicos detectados del conjunto de mediciones"
#| label: fig-outliers
...
outliers = labels == -1
ax.scatter(datos['Sensor'][outliers], datos['Camara'][outliers], color='red', s=25, edgecolors='k', label="Valores atípicos")

# Configurar la leyenda y los ejes
ax.set_xlabel("Sensor [°C]")
ax.set_ylabel("Cámara [°C]")
ax.grid(True)
ax.legend()
plt.legend(loc='lower right')
plt.show()
...
Posteriormente, se utiliza el método de calibración de punto, el cual ajusta el error a través de un factor de corrección constante para alinear las mediciones del sensor con un estándar de referencia, en este caso las mediciones de la cámara. Para obtener este factor se usa un método iterativo que evalúa diferentes factores de corrección dentro de un rango definido. Este rango se define utilizando el EAM y la desviación estándar de los errores (𝜎). El margen de error se calcular siguiendo la @eq-margen-error, donde $Z$ es el $Z_{score}$ que se considera igual a 2.58 para un nivel de confianza del 99% y n es el número de muestras.

$$
M = Z \times \left( \frac{\sigma}{\sqrt{n}} \right)
$$ {#eq-margen-error}

Para el cálculo de los límites inferior y superior del intervalo de confianza, se utilizan las  ecuaciones @eq-lim-sup y @eq-lim-inf:
...
Una vez definido el rango, se lleva a cabo el método iterativo con un intervalo de 0.01 para probar distintos factores de corrección. Se obtiene el factor de corrección óptimo en 0.47

Para validar el factor de corrección se aplica al conjunto de datos sin eliminar los valores atípicos siguiendo la @eq-validacion-factor-1, donde $M_c$ son las mediciones corregidas, $M_s$ son las mediciones hechas por el sensor GY-906 y $f_c$ es el factor de corrección
...
Al Aplicar el factor de corrección al conjunto de datos, el $ME$ disminuye hasta -0.25°C, mientras que el $EAM$ disminuye hasta XXX°C. Posteriormente se lleva cabo una segunda campaña de medición con el objetivo de validar el factor de corrección y se obtienen un total de 25 pares de mediciones. En este nuevo conjunto de datos se obtiene un $EM = -0.3$, el cual al aplicar el factor de corrección disminuye a 0.17. Por otro lado, se obtiene un $EAM = 0.40$, el cual disminuye a 0.34 tras aplicar el factor de corrección. Logrando una mejora en la precisión de las mediciones del sensor.




## Lógica de programación {#sec-programación}


En este apartado se describe la lógica de programación desarrollada para el reloj inteligente, un dispositivo diseñado para realizar encuestas de confort térmico simplificadas y medir variables fisiológicas como la temperatura de la piel en la muñeca y la frecuencia cardíaca. La programación se lleva a cabo en Arduino, principalmente mediante el uso de la librería LVGL para gestionar la interfaz gráfica. El dispositivo cuenta con un microcontrolador XIAO ESP32C3 y una pantalla táctil, elementos que permiten la interacción directa con el usuario.

El proyecto consta de varios archivos de código, cada uno con una función específica dentro del sistema. A continuación, se presenta una breve descripción de cada archivo:

1. **Proyecto\_confort.ino**: Este es el archivo principal del proyecto. En él se ejecuta el código base, se incluyen las librerías necesarias y se declaran variables y funciones de importancia.
...
3. **ui\_Aceptación.c**: Configura la pantalla destinada a la pregunta de aceptación térmica, donde el usuario evalúa si las condiciones térmicas le resultan agradables o no.

4. **ui\_Clo.c**: Configura la pantalla para la pregunta sobre el nivel de aislamiento térmico de la ropa que usa el usuario, lo cual contribuye al cálculo del confort térmico.

5. **ui\_Inicio.c**: Configura la pantalla principal del reloj. En esta pantalla se muestra la hora actual, un contador regresivo que indica cuánto tiempo falta hasta el momento de contestar la siguiente encuesta, y se habilita el botón para iniciar la encuesta cuando está disponible.

6. **ui\_Met.c**: Configura la pantalla para la pregunta sobre el nivel de actividad metabólica del usuario, considerando si se encuentra en reposo, caminando, o realizando alguna actividad física.

7. **ui\_Sensacion.c**: Configura la pantalla para la pregunta sobre el nivel de sensación térmica, donde el usuario describe su percepción del ambiente como frío, cálido o neutral.

8. **ui\_Ubicación.c**: Configura la pantalla para la pregunta sobre la ubicación del usuario, lo cual ayuda a contextualizar las condiciones ambientales que están siendo evaluadas.

9. **ui\_events.c** y **ui\_events.h**: En **ui\_events.c** se encuentran la mayoría de las funciones declaradas para el programa, incluyendo aquellas que gestionan los eventos generados por la interfaz. El archivo header (**ui\_events.h**) contiene las declaraciones de dichas funciones.

10. **ui\_helpers.c** y **ui\_helpers.h**: Contienen funciones de ayuda utilizadas para facilitar la implementación de la interfaz y realizar operaciones comunes dentro del sistema.

La lógica de programación del dispositivo está diseñada para realizar encuestas de confort térmico entre las 8 de la mañana y las 10 de la noche, en intervalos de media hora. Esto se hace para asegurar que las encuestas se realicen después de un periodo de adaptación climática del usuario en el Instituto de Energías Renovables (IER), donde se lleva a cabo el proyecto. La función denominada **"Actualizar\_hora"**, que se encuentra en el archivo **ui\_events.c**, se encarga de actualizar la hora en pantalla y, además, habilitar o deshabilitar el botón para iniciar la encuesta según el horario. Esta función también emite una alarma vibratoria para alertar al usuario sobre la disponibilidad de una nueva encuesta. La lógica detallada de esta función se representa en la @fig-actualizarhora.

![Diagrama de flujo de la función Actulizar hora](../Imagenes/Actualizar_hora.svg){#fig-actualizarhora width=50%}

Para ahorrar energía, la pantalla del dispositivo permanece apagada mientras el reloj está en reposo. Además, si la pantalla está encendida y el dispositivo detecta inactividad durante más de 10 segundos, esta se apaga automáticamente. Sin embargo, la pantalla puede ser encendida nuevamente al hacer clic sobre ella, lo cual permite al usuario ver la hora en cualquier momento.

Una vez que el usuario inicia la encuesta, el proceso implica responder a las preguntas de confort térmico, registrar las variables fisiológicas y validar las mediciones. Las variables se actualizan de manera continua a medida que el usuario responde cada pregunta. Al finalizar la encuesta y enviar los datos a la plataforma IoT **Thingsboard** para su análisis, las variables no se reinician. Esto significa que, al iniciar una nueva encuesta, el dispositivo retiene las respuestas de la última encuesta contestada, permitiendo al usuario simplemente continuar con las mismas respuestas o realizar modificaciones si lo requiere. Este enfoque tiene dos propósitos principales: en primer lugar, ahorrar tiempo al contestar encuestas sucesivas, y en segundo lugar, permitir que el usuario observe cómo cambian sus respuestas a lo largo del día. En caso de que el usuario no acepte las mediciones, estas se toman nuevamente. El proceso completo de la encuesta se describe en la @fig-iniciarencuesta.

![Diagrama de flujo de encuesta y medición de variables](../Imagenes/Iniciar_encuesta.svg){#fig-iniciarencuesta}

Los diagramas de flujo proporcionan una representación visual de la secuencia de operaciones, ayudando a comprender cómo las funciones interactúan entre sí para llevar a cabo las encuestas y asegurar la correcta recolección de datos. La lógica del programa está centrada en la simplicidad de uso y en la eficiencia del proceso de toma de decisiones, siempre con el objetivo de minimizar la interacción innecesaria del usuario y maximizar la calidad de los datos recolectados.

En la siguiente sección, "Instrucciones de uso", se detallan las configuraciones iniciales necesarias para el funcionamiento del dispositivo, tales como la conexión a la red WiFi, la configuración del token del dispositivo y la configuración del servidor de **Thingsboard**. Estas configuraciones son fundamentales para asegurar la conectividad y el correcto funcionamiento del dispositivo durante la recolección y envío de datos.
